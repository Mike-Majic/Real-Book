...
function buildDeck() {
  return shuffle([...EMOJIS, ...EMOJIS]).map((symbol, i) => ({ id: i, symbol }));
}
...
export default function Memory({ onFinish }) {
  ...
  const [deck] = useState(buildDeck); // fissato all'avvio della partita
  ...
  const finishGame = (finalMoves, finalScores) => {
    if (mode === 'solo') {
      const score = Math.max(0, 200 - finalMoves * 10);
